...
class EntidadesTecnicas(BaseModel):
    ...
    @classmethod
    def sin_duplicados_ni_vacios(cls, value: List[str]) -> List[str]:
        limpio = [tecnologia.strip() for tecnologia in value if tecnologia.strip()]
        if not limpio:
    ...
    def resumen_no_vacio(cls, value: str) -> str:
        resumen = value.strip()
        if len(resumen) < 10:
            raise ValueError("El resumen técnico debe tener al menos 10 caracteres")
        return resumen
